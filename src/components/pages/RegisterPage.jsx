    ...
    <>
      <div className='fon' style={{ background: 'none' }}>
        <div className='register-form'>
          <button
            className='close'
            style={{ left: '368px', top: '8px', border: 'none', background: 'none' }}
            onClick={() => navigate('/')}
          >
            <CloseIcon />
          </button>
          ...
          <input
            type='text'
            className='inp-email-pochta'
            placeholder='Введите почту'
            value={regEmailInpValue}
            onChange={(e) => setRegEmailInpValue(e.target.value)}
          />
          <p className='title-passwords'>Придумайте пароль, пожалуйста:</p>
          <input
            type='text'
            className='inp-passwords'
            placeholder='Введите'
            value={regPasswordInpValue}
            onChange={(e) => setRegPasswordInpValue(e.target.value)}
          />
          <button className='registr-voiti-btn' onClick={createUser}>
            Регистрация
          </button>
          <button className='have-accaunt-btn' onClick={() => navigate('/login')}>
            Уже есть аккаунт?
          </button>
